#!/usr/bin/env python3
"""Generate pLaplace final report figures from the completed suite."""
...
SUMMARY_JSON = REPO_ROOT / "experiment_results_cache/plaplace_final_suite/summary.json"
...
def _plot_scaling(rows: list[dict]) -> Path:
    ...
    out_path = OUT_DIR / "plaplace_scaling.png"
...
def _plot_dof_runtime(rows: list[dict], nprocs: int = 8) -> Path:
    ...
    out_path = OUT_DIR / f"plaplace_dof_runtime_np{nprocs}.png"
    fig.savefig(out_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
    return out_path
...
def _plot_fine_convergence(rows: list[dict], level: int = 9, nprocs: int = 32) -> Path:
    ...
    ax.grid(True, which="both", linestyle=":", alpha=0.5)
    ax.legend(frameon=False)
    out_path = OUT_DIR / f"plaplace_convergence_l{level}_np{nprocs}.png"
    fig.savefig(out_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
    return out_path
