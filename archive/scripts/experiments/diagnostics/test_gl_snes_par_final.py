#!/usr/bin/env python3
"""Test basic+gmres+hypre+divtol=-1 at various proc counts."""
import sys
import numpy as np
...
if rank == 0:
    sys.stdout.write(f"GL SNES basic+gmres+hypre+divtol=-1 | {comm.size} procs\n")
    sys.stdout.flush()

for mesh_level in [5, 6, 7, 8, 9]:
    N = 2 ** (mesh_level + 1)
    ...
    # Use unique prefix per level to avoid option collisions
    pfx = f"gls{mesh_level}_"
    try:
        ...
        if rank == 0:
            sys.stdout.write(f"  L{mesh_level} dofs={V.dofmap.index_map.size_global}: {dt:.2f}s iters={n_iters} reason={reason} J={final_energy:.6f} {'OK' if ok else 'FAIL'}\n")
            sys.stdout.flush()
