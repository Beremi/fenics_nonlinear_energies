...
files = [
    ...
    ("JAX (1e-1)", "l4_jax_gamg_1e-1.json"),
    ("FEniCS (1e-3)", "l4_fenics_exact_gamg.json"), 
    ("JAX (1e-3)", "l4_jax_exact_gamg.json"),
    ("FEniCS (1e-6)", "l4_fenics_gamg_1e-6.json"), 
    ("JAX (1e-6)", "l4_jax_gamg_1e-6.json")
]

print(f"{'Implementation':<15} | {'Final Energy':<15} | {'Newton Iter':<17} | {'Total KSP Iter':<15} | {'Solve Time'}")
print("-" * 83)
for label, f in files:
    res = parse_file(f)
    if "Error" in res:
        print(f"{label:<15} | Running... ({f})")
    else:
        print(f"{label:<15} | {res['Final Energy']:<15} | {str(res['Newton Iters']):<17} | {str(res['Linear Iters']):<15} | {res['Time']}")
