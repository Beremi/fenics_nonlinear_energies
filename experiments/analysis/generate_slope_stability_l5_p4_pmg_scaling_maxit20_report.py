#!/usr/bin/env python3
"""Generate a scaling report for the featured L5 P4 PMG solver with Newton maxit=20."""
...
DEFAULT_INPUT = Path("artifacts/raw_results/slope_stability_l5_p4_pmg_scaling_lambda1_maxit20/summary.json")
DEFAULT_OUTPUT = Path("artifacts/reports/slope_stability_l5_p4_pmg_scaling_lambda1_maxit20")
...
def _plot(rows: list[dict[str, object]], out_dir: Path) -> None:
    ...
    ax.set_title("L5 P4 PMG timing, maxit=20 (log-log)")
    ax.set_xticks(ranks)
    ...
    ax.set_title("L5 P4 PMG strong scaling, maxit=20")
    ax.set_xticks(ranks)
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def main() -> None:
    ...
    report = f"""# `L5` `P4` PMG Scaling With Newton `maxit=20`
...
This sweep uses the same top PMG solver as the model card, but limits the
outer Newton loop to `20` iterations and expands the rank ladder to
`1/2/4/8/16/32`.

Only the `4`-rank run satisfies the full nonlinear stop test under this cap.
The speedup column therefore uses `4` ranks as its baseline and the other rows
show how quickly the capped run progresses rather than a fully converged
...
![L5 P4 PMG scaling, maxit=20](scaling.png)
"""
