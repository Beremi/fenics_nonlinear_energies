#!/usr/bin/env python3
"""Generate pLaplace overview PDF figures."""
...
def _state_figure():
    ...
    out = family_pdf(f"{FAMILY}/plaplace_sample_state.pdf")
    save_pdf(fig, out)
    record_provenance(
        family_data(f"{FAMILY}/plaplace_sample_state.provenance.json"),
        script_name="experiments/analysis/docs_assets/build_plaplace_figures.py",
...
def _energy_figure():
    ...
    out = family_pdf(f"{FAMILY}/plaplace_energy_levels.pdf")
    save_pdf(fig, out)
    record_provenance(
        family_data(f"{FAMILY}/plaplace_energy_levels.provenance.json"),
        script_name="experiments/analysis/docs_assets/build_plaplace_figures.py",
        inputs=[repo_rel(family_data(f"{FAMILY}/energy_levels.csv"))],
        outputs=[repo_rel(out)],
        notes="Energy-vs-level figure from the authoritative maintained pLaplace suite.",
    )
...
def _strong_scaling_figure():
    ...
    by_impl: dict[str, list[dict[str, str]]] = {}
    for row in rows:
    ...
    out = family_pdf(f"{FAMILY}/plaplace_strong_scaling.pdf")
    save_pdf(fig, out)
    record_provenance(
        family_data(f"{FAMILY}/plaplace_strong_scaling.provenance.json"),
        script_name="experiments/analysis/docs_assets/build_plaplace_figures.py",
        inputs=[repo_rel(family_data(f"{FAMILY}/strong_scaling.csv"))],
        outputs=[repo_rel(out)],
        notes="Strong scaling on the finest maintained pLaplace mesh (level 9) with an ideal 1/r reference.",
    )
...
def _mesh_timing_figure():
    ...
    out = family_pdf(f"{FAMILY}/plaplace_mesh_timing.pdf")
    save_pdf(fig, out)
    record_provenance(
        family_data(f"{FAMILY}/plaplace_mesh_timing.provenance.json"),
        script_name="experiments/analysis/docs_assets/build_plaplace_figures.py",
        inputs=[repo_rel(family_data(f"{FAMILY}/mesh_timing.csv"))],
        outputs=[repo_rel(out)],
        notes="Time-vs-mesh-size scaling on the maintained pLaplace suite at 32 MPI ranks with an ideal linear reference.",
    )
