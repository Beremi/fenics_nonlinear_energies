#!/usr/bin/env python3
"""Generate a per-part scaling report for the instrumented L4 P4 PMG solve."""
...
def _aggregate_mg_runtime(linear_records: list[dict[str, object]]) -> dict[str, float]:
    totals: dict[str, float] = {
        ...
        "transfer_l3p1_to_l4p1_prolong_time_sec": 0.0,
        "transfer_l4p1_to_l3p1_restrict_time_sec": 0.0,
        "transfer_l4p1_to_l4p2_prolong_time_sec": 0.0,
        "transfer_l4p2_to_l4p1_restrict_time_sec": 0.0,
        "transfer_l4p2_to_l4p4_prolong_time_sec": 0.0,
        "transfer_l4p4_to_l4p2_restrict_time_sec": 0.0,
    }
    for record in linear_records:
        ...
        for diag in record.get("mg_runtime_diagnostics", []):
            ...
            if kind == "transfer":
                ...
                detail_key = (
                    f"transfer_l{target_mesh_level}p{target_degree}_to_l{mesh_level}p{degree}_"
                    f"{'prolong' if role == 'prolongation' else 'restrict'}_time_sec"
                )
...
def main() -> None:
    ...
    _plot_loglog(
        rows,
        keys=[
            ("transfer_l3p1_to_l4p1_prolong_time_sec", "L3 P1 -> L4 P1"),
            ("transfer_l4p1_to_l4p2_prolong_time_sec", "L4 P1 -> L4 P2"),
            ("transfer_l4p2_to_l4p4_prolong_time_sec", "L4 P2 -> L4 P4"),
        ],
    ...
    row0 = rows[0]
    lines: list[str] = []
    lines.append("# L4 P4 PMG Part Scaling")
    lines.append("")
    lines.append(
        "This report uses an instrumented explicit-Galerkin PMG equivalent of the best current "
        "L4 `P4` hierarchy so we can time transfer applications, smoother work, and the coarse solve separately."
    )
    lines.append(
        "The key point is that the measured run keeps the same hierarchy and smoother family as the best legacy PMG, "
        "but exposes the internal MG parts directly."
    )
    lines.append("")
    ...
    lines.append("- interpreted as same-mesh `P4 -> P2 -> P1` plus the `L3 P1` tail")
    ...
    lines.append("")
    lines.append("| ranks | L3 P1 -> L4 P1 [s] | L4 P1 -> L4 P2 [s] | L4 P2 -> L4 P4 [s] |")
    lines.append("| ---: | ---: | ---: | ---: |")
    for row in rows:
        lines.append(
            f"| {row['ranks']} | {_fmt(row['transfer_l3p1_to_l4p1_prolong_time_sec'])} | "
            f"{_fmt(row['transfer_l4p1_to_l4p2_prolong_time_sec'])} | "
            f"{_fmt(row['transfer_l4p2_to_l4p4_prolong_time_sec'])} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        f"- On `1` rank, the instrumented PMG reproduces the legacy baseline outer path exactly: "
        f"`{rows[0]['newton_iterations']}` Newton steps and `{rows[0]['linear_iterations']}` total outer linear iterations."
    )
    legacy_match = all(
        row["legacy_newton_iterations"] == row["newton_iterations"]
        and row["legacy_linear_iterations"] == row["linear_iterations"]
        for row in rows
        if row["legacy_newton_iterations"] is not None
    )
    if legacy_match:
        lines.append(
            "- Across `1/2/4/8` ranks, the instrumented explicit-Galerkin PMG matches the legacy baseline "
            "Newton and outer-linear iteration counts."
        )
    else:
        lines.append(
            "- The instrumented explicit-Galerkin PMG is close to the legacy baseline, but not identical on every rank."
        )
    best8 = rows[-1]
    lines.append(
        f"- At `8` ranks, the dominant timed MG pieces are `P4` smoothing `{_fmt(best8['p4_smoother_time_sec'])} s`, "
        f"`P2` smoothing `{_fmt(best8['p2_smoother_time_sec'])} s`, `P1` smoothing `{_fmt(best8['p1_smoother_time_sec'])} s`, "
        f"and coarse solve `{_fmt(best8['coarse_solve_time_sec'])} s`."
    )
    lines.append(
        f"- Transfer application cost is much smaller: prolongation `{_fmt(best8['transfer_prolongation_time_sec'])} s` "
        f"and restriction `{_fmt(best8['transfer_restriction_time_sec'])} s` on `8` ranks."
    )
    lines.append(
        f"- The setup-like MG cost that still matters inside the linear phase is explicit Galerkin refresh: "
        f"`{_fmt(best8['galerkin_refresh_time_sec'])} s` on `8` ranks."
    )
    lines.append("")
    lines.append("## Plots")
    lines.append("")
    lines.append("- `overall_scaling_loglog.png`: total and solve scaling")
    lines.append("- `pc_transfer_scaling_loglog.png`: transfer and PC-maintenance scaling")
    lines.append("- `pc_smoother_scaling_loglog.png`: smoother and coarse-solve scaling")
    lines.append("- `prolongation_detail_scaling_loglog.png`: per-transfer prolongation scaling")
    lines.append("")
