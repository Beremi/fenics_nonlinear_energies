#!/usr/bin/env python3
"""Generate a scaling report for the L6 P4 deep-P1-tail PMG solver with Newton maxit=20."""
...
DEFAULT_INPUT = Path("artifacts/raw_results/slope_stability_l6_p4_deep_p1_tail_scaling_lambda1_maxit20/summary.json")
DEFAULT_OUTPUT = Path("artifacts/reports/slope_stability_l6_p4_deep_p1_tail_scaling_lambda1_maxit20")
...
def _plot(rows: list[dict[str, object]], out_dir: Path) -> None:
    ...
    ax.set_xlabel("MPI ranks")
    ax.set_ylabel("time [s]")
    ax.set_title("L6 P4 deep-P1-tail PMG timing (log-log)")
    ax.set_xticks(ranks)
    ax.get_xaxis().set_major_formatter(plt.ScalarFormatter())
    ax.grid(True, alpha=0.3)
    ax.legend()
    ...
    ax.set_title("L6 P4 deep-P1-tail strong scaling")
    ax.set_xticks(ranks)
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def main() -> None:
    ...
    _plot_detail_group(
        ...
        setup_parts,
        setup_values,
        title="L6 P4 deep-P1-tail setup subparts (log-log)",
    ...
    _plot_detail_group(
        ...
        callback_parts,
        callback_values,
        title="L6 P4 deep-P1-tail callback totals (log-log)",
    ...
    _plot_detail_group(
        ...
        linear_parts,
        linear_values,
        title="L6 P4 deep-P1-tail final linear solve (log-log)",
    ...
    _plot_detail_group(
        ...
        mg_parts,
        mg_values,
        title="L6 P4 deep-P1-tail PMG internals (log-log)",
        output_path=out_dir / "pmg_internal_loglog.png",
    )

    hierarchy = rows[0].get("mg_custom_hierarchy", "1:1,2:1,3:1,4:1,5:1,6:1,6:2,6:4") if rows else ""
    report = f"""# `L6` `P4` Deep-`P1`-Tail PMG Scaling With Newton `maxit=20`
...
This is the winning progression from the `L6` hierarchy search, benchmarked on
`1/2/4/8` MPI ranks for `20` Newton iterations.
...
![L6 P4 deep-P1-tail setup subparts](setup_subparts_loglog.png)
...
![L6 P4 deep-P1-tail callback breakdown](callback_breakdown_loglog.png)
...
![L6 P4 deep-P1-tail final linear solve breakdown](linear_breakdown_loglog.png)
...
![L6 P4 deep-P1-tail PMG internal breakdown](pmg_internal_loglog.png)

## Graph

![L6 P4 deep-P1-tail PMG scaling, maxit=20](scaling.png)
"""
