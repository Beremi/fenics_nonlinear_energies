#!/usr/bin/env python3
"""Generate GL final-report figures from the validated final campaign."""
...
DEFAULT_ASSET_DIR = REPO_ROOT / "docs" / "assets" / "ginzburg_landau"
...
def _plot_scaling(rows: list[dict], asset_dir: Path) -> Path:
    ...
    out_path = asset_dir / "gl_scaling.png"
...
def _plot_dof_runtime(rows: list[dict], asset_dir: Path, nprocs: int = 8) -> Path:
    ...
    out_path = asset_dir / f"gl_dof_runtime_np{nprocs}.png"
    fig.savefig(out_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
    return out_path
...
def _plot_fine_convergence(rows: list[dict], asset_dir: Path, level: int = 9, nprocs: int = 32) -> Path:
    ...
    ax.grid(True, which="both", linestyle=":", alpha=0.5)
    ax.legend(frameon=False)
    out_path = asset_dir / f"gl_convergence_l{level}_np{nprocs}.png"
    fig.savefig(out_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
    return out_path
