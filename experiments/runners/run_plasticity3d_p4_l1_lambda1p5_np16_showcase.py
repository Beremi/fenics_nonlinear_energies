#!/usr/bin/env python3
"""Run the canonical Plasticity3D `P4(L1)` docs showcase at `lambda = 1.5`."""
...
DEFAULT_RAW_DIR = (
    ...
    / "raw_results"
    / "docs_showcase"
    / "plasticity3d_p4_l1_lambda1p5_np16_grad1e2"
)
CANONICAL_DIR = (
    REPO_ROOT
    / "artifacts"
...
def _refresh_canonical(*, raw_dir: Path, progress_json: Path, showcase_meta: Path) -> None:
    CANONICAL_DIR.mkdir(parents=True, exist_ok=True)
    shutil.copy2(raw_dir / "state.npz", CANONICAL_DIR / "state.npz")
    shutil.copy2(progress_json, CANONICAL_DIR / "progress.json")
    shutil.copy2(showcase_meta, CANONICAL_DIR / "showcase_meta.json")
    output_payload = read_json(raw_dir / "output.json")
    output_payload["state_out"] = str((CANONICAL_DIR / "state.npz").resolve())
    write_json(CANONICAL_DIR / "output.json", output_payload)
...
def main() -> None:
    ...
    run_logged_command(
        ...
        notes="Authoritative 16-rank maintained-local Plasticity3D P4(L1) lambda=1.5 showcase run.",
    )

    payload = read_json(output_json)
    if str(payload.get("status", "")).strip().lower() != "completed":
        raise RuntimeError(f"Showcase run did not complete successfully: {payload.get('status')} / {payload.get('message')}")
    if float(payload.get("lambda_target", float("nan"))) != 1.5:
    ...
    if float(payload.get("final_grad_norm", float("inf"))) >= 1.0e-2:
        raise RuntimeError("Showcase run did not satisfy the target gradient norm")
    ...
    showcase_meta = _write_showcase_meta(
        out_dir=out_dir,
        command=command,
        env=env,
    ...
    if bool(args.refresh_canonical):
        _refresh_canonical(raw_dir=out_dir, progress_json=progress_json, showcase_meta=showcase_meta)
