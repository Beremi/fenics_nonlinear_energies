#!/usr/bin/env python3
"""Run a preserved Plasticity3D `P1(L1_2_3)` showcase at `lambda = 1.55` on 32 MPI ranks."""
...
DEFAULT_RAW_DIR = (
    ...
    / "raw_results"
    / "docs_showcase"
    / "plasticity3d_p1_l1_2_3_lambda1p55_np32_grad1e2"
)
...
def _write_progress_json(*, output_payload: dict[str, object], out_path: Path) -> None:
    progress = {
        ...
        "elem_degree": int(output_payload.get("elem_degree", 1)),
...
def main() -> None:
    ...
    run_logged_command(
        ...
        notes="Authoritative 32-rank maintained-local Plasticity3D P1(L1_2_3) lambda=1.55 showcase run.",
    )
    ...
    if str(payload.get("mesh_name", "")) != "hetero_ssr_L1_2_3":
        raise RuntimeError("Showcase output recorded an unexpected mesh_name")
    if int(payload.get("elem_degree", 0)) != 1:
