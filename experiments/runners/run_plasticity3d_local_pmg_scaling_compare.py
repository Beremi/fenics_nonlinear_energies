#!/usr/bin/env python3
"""Run focused scaling for two backend-mix local PMG Plasticity3D variants."""
...
DEFAULT_OUT_DIR = (
    ...
    / "raw_results"
    / "source_compare"
    / "plasticity3d_local_pmg_scaling_tol1e1"
)
DEFAULT_RANKS = (1, 2, 4, 8, 16, 32)
SUMMARY_NAME = "comparison_summary.json"
RUNNER_NAME = "plasticity3d_local_pmg_scaling_compare"

IMPLEMENTATIONS = (
    {
        "name": "local_constitutiveAD_local_pmg_tol1e1",
        ...
        "solver_profile": "local_pmg_rtol_1e-1",
    },
    {
        "name": "source_local_pmg_tol1e1",
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run strong scaling for local PMG backend-mix rows at rtol=1e-1."
    )
...
def main() -> None:
    ...
    env["PETSC_OPTIONS"] = "-mix_init_ksp_rtol 1e-1 -mix_newton_ksp_rtol 1e-1"
    ...
    for ranks in [int(v) for v in args.ranks]:
        for impl in IMPLEMENTATIONS:
            ...
            command = mix_tools._build_case_command(
            ...
            if exit_code != 0 or not result_path.exists():
                message = mix_tools._tail_text(stderr_path) or mix_tools._tail_text(stdout_path) or "subprocess failed"
                row = _failed_row(
                    impl=impl,
                    case_id=case_id,
                    ranks=int(ranks),
