...
from topological_optimisation_jax.solve_topopt_jax import run_topology_optimisation


REPO_ROOT = Path(__file__).resolve().parent.parent
ASSET_DIR = Path(__file__).resolve().parent / "report_assets"
REPORT_PATH = REPO_ROOT / "jax_topology_optimisation_benchmark.md"
...
REPORT_GENERATOR_CMD = "./.venv/bin/python topological_optimisation_jax/generate_report_assets.py"
GIF_MAX_FRAMES = 80
...
def _solver_command(params: dict) -> str:
    return "\n".join(
        [
            ...
            f"    --ksp_rtol {params['ksp_rtol']} --ksp_max_it {params['ksp_max_it']} --save_outer_state_history --quiet",
        ]
    )
...
def generate_benchmark_assets(
    *,
    ...
    asset_dir = ASSET_DIR if asset_dir is None else Path(asset_dir)
    report_path = REPORT_PATH if report_path is None else Path(report_path)
    ...
    try:
        ...
        report_path.write_text(_make_report(result, state, mesh))
...
def main() -> None:
    _, _, _ = generate_benchmark_assets()
