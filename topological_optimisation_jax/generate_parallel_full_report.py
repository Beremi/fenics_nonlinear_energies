...
MAX_GIF_FRAMES = 120
DEFAULT_ASSET_DIR = REPO_ROOT / "topological_optimisation_jax" / "report_runs" / "parallel_full"
...
def _solver_command(result: dict, snapshot_dir: Path, run_json: Path, state_npz: Path) -> str:
    ...
    return "\n".join(
        [
            f"mpiexec -n {result['nprocs']} ./.venv/bin/python topological_optimisation_jax/solve_topopt_parallel.py \\",
