...
from tools_petsc4py.minimizers import gradient_descent
from topological_optimisation_jax.jax_energy import theta_from_latent
from topological_optimisation_jax.parallel_support import (
...
def _relative_state_change(current: np.ndarray, previous: np.ndarray | None, freedofs: np.ndarray) -> float:
    if previous is None:
        return np.inf
    return float(
...
def _design_step_is_acceptable(
    ...
    *,
    ...
    msg = message.lower()
    if _message_is_converged(message):
        return True
