...
USER_AGENT = "fenics_nonlinear_energies-literature-audit/1.0"
...
def download_fulltext(url: str, destination: Path) -> None:
    ...
    response = requests.get(url, headers={"User-Agent": USER_AGENT}, timeout=60, allow_redirects=True)
...
def main() -> None:
    ...
    if manifest_key_set != cited_key_set:
        missing = sorted(cited_key_set - manifest_key_set)
        extra = sorted(manifest_key_set - cited_key_set)
        raise SystemExit(f"Manifest keys do not match cited keys. Missing={missing}, extra={extra}")
    ...
    for key in cited_keys:
        if key not in bib_entries:
            raise SystemExit(f"Cited key {key!r} is missing from {args.bib}")
    ...
    if sorted(covered_keys) != sorted(cited_keys):
        raise SystemExit("Coverage mismatch between extracted citations and generated literature tables")
    if len(set(covered_keys)) != len(cited_keys):
        raise SystemExit("Some cited keys appeared more than once across the generated literature tables")

    lines = [
        "# Literature Sources",
        "",
        "Generated by `python scripts/generate_literature_sources.py` from `paper/references.bib` and `paper/literature/manifest.json`.",
        "",
        *render_table(
            available_rows,
            "## Available full text",
