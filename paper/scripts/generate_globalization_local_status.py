#!/usr/bin/env python3
"""Generate the bounded-outcome EXP-GLOB-001 status table and manifest."""
...
def build_manifest(
    *,
    ...
    source = evidence_root / CAMPAIGN_MANIFEST_RELATIVE
    paths = {
        "validator": repo_root / "paper/scripts/globalization_local_evidence.py",
        "generator": repo_root / "paper/scripts/generate_globalization_local_status.py",
        "checker": repo_root / "paper/scripts/check_globalization_local_manifest.py",
    }
    tools = {
        key: {
    ...
    return {
        ...
        "source_campaign_manifest": {
            "path": _repo_relative(source, repo_root=repo_root),
...
def generate(
    ...
    *,
    ...
    if sha256_file(table_path) != table_sha256:
        raise OSError("written globalization status table differs from generated bytes")
