...
| Validation: Hyperelasticity | The specific JAX-FEM companion case uses the same mesh and displacement schedule, a logarithmic-$J$ Piola solve law, and common endpoint-energy evaluation under the paper hyperelastic energy | curated paper-submission comparison summary and run manifest | `artifacts/reproduction/paper_submission_2026_07_08/inputs/jax_fem_hyperelastic_baseline/comparison_summary.json` fairness gate and `run_manifest.json` case/schedule records |
| Validation: Plasticity3D | The seven-step endpoint-observable comparison uses a common local endpoint integrand, mesh, material table, Davis-B strength reduction, and load schedule but different constrained spaces; the fixed-`lambda` diagnostic additionally matches the active free-DOF set | curated paper-submission comparison summary and validation manifest | `artifacts/reproduction/paper_submission_2026_07_08/inputs/plasticity3d_validation/comparison_summary.json` and `validation_manifest.json`; used for endpoint-observable provenance, not as validation of incremental plastic-history paths |
...
| Introduction, Related Work | Per-element forward-mode AD and sparse Hessian assembly for locality-aware GPU mesh computations | `mahmoud2025locality` | arXiv abstract and metadata for arXiv:2509.00406; preprint evidence only |
| Introduction, Related Work | Globally applied AD with matrix-free products and coloring-based sparse tangent materialization in the `tatva` finite-element framework | `pundir2026tatva` | arXiv abstract and metadata for arXiv:2602.12365; preprint evidence only |
| Introduction, Related Work | Automatic local/global sparsity detection and coloring-enabled sparse Jacobian/Hessian differentiation | `hill2025sparser` | TMLR/OpenReview accepted record and arXiv:2501.17737 abstract |
...
| Related Work, Methodology | PETSc as scalable vector/matrix/Krylov/nonlinear infrastructure | `balay1997petsc`, `petsc2026manual` | PETSc chapter and official PETSc/TAO Users Manual citation block, accessed 2026-04-30 |
...
| Related Work, Benchmarks: Plasticity2D / Plasticity3D | Source-record evidence for a Mohr--Coulomb return-mapping paper in the Sysala slope-stability reference family | `sysala2017returnmapping` | DOI and CAS ASEP metadata only; CAS ASEP lists a restricted postprint and no public full text is cached. Used only for reference-family context, not for detailed method claims or validation of the endpoint surrogate |
...
| Methodology | Armijo backtracking, line-search Newton, and trust-region globalization terminology | `nocedal2006numerical`, `conn2000trust` | Nocedal--Wright Chapters 2--4, especially Section 3.1, and Conn--Gould--Toint trust-region monograph metadata |
| Results: Hyperelasticity | Karolina CPU-node hardware description for the fixed-work multi-node runs | `it4i-karolina-compute-nodes` | Official IT4Innovations Karolina compute-node documentation; used only for processor, core-count, and memory description of the reported runs |
