...
| Validation: Plasticity3D | The seven-step endpoint-observable comparison uses a matched endpoint functional, mesh, material table, Davis-B strength reduction, load schedule, and boundary-condition convention; the fixed-`lambda` diagnostic additionally matches the active free-DOF set | curated paper-submission comparison summary and validation manifest | `artifacts/reproduction/paper_submission_2026_07_08/inputs/plasticity3d_validation/comparison_summary.json` and `validation_manifest.json`; used for endpoint-observable provenance, not as validation of incremental plastic-history paths |
