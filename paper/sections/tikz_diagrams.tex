...
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[paper box, fill=paperdiagbeige, text width=3.7cm] (ref) at (0,0)
    {\textbf{Reference}\\\textbf{implementations}\\pure JAX, FEniCS};
  \node[paper box, fill=paperdiagblue, text width=3.8cm] (main) at (5.7,0)
    {\textbf{JAX+PETSc}\\\textbf{realization}\\Newton--Krylov with Hypre/GAMG/PMG};
  \node[paper box, fill=paperdiaggreen, text width=3.85cm] (bench) at (11.4,0)
    {\textbf{Benchmark families}\\scalar PDEs, mechanics, topology};
  \node[paper box, fill=paperdiagviolet, text width=4.55cm] (deriv) at (3.0,-2.0)
    {\textbf{Derivative modes}\\element AD, constitutive AD, colored recovery};
  \node[paper box, fill=paperdiagrose, text width=4.15cm] (solver) at (8.7,-2.0)
    {\textbf{Solver modes}\\line search, trust region, multigrid};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[paper box, fill=paperdiaggreen, text width=3.55cm] (petsc) at (11.2,0)
    {\textbf{PETSc algebra}\\distributed Newton driver};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[paper box, fill=paperdiagblue, text width=15.2cm] (e) at (0,0)
    {\textbf{Element AD}\\$\Pi_e(u_e)$; active-branch local gradient and Hessian; higher-order elements may be costly};
  \node[paper box, fill=paperdiaggreen, text width=15.2cm, below=0.45cm of e] (c)
    {\textbf{Constitutive AD}\\$\mathcal{W}_q(\varepsilon_q,m_q)$; quadrature-point tangent assembly; branchwise AD constitutive tangents};
  \node[paper box, fill=paperdiagrose, text width=15.2cm, below=0.45cm of c] (s)
    {\textbf{Colored recovery}\\rank-local AD-HVP probes in reported cases; parallel coloring / recovery; finite-difference gradient probes as the classical variant};
\end{tikzpicture}%
}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[paper box, fill=paperdiaggreen, text width=15.2cm, below=0.45cm of ls] (tr)
    {\textbf{Trust region}\\quadratic-model step; $\rho$-based acceptance; radius shrink / expand};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[paper box, fill=paperdiaggray, text width=4.0cm] (patternbox) at (0.0,0.0)
    {\textbf{1. Sparse coupling}\\\textbf{pattern}\\Nearby DOFs interfere; far-away columns do not.};
  \node[paper box, fill=paperdiagviolet, text width=4.0cm] (colorbox) at (5.6,0.0)
    {\textbf{2. Conflict coloring}\\Noninterfering columns share one probe color.};
  \node[paper box, fill=paperdiaggreen, text width=4.0cm] (recoverbox) at (11.2,0.0)
    {\textbf{3. Probe and recover}\\One directional probe recovers several sparse columns.};
  ...
  \begin{scope}[shift={(3.95,-1.78)}]
    ...
    \node[font=\footnotesize,align=center] at (1.56,-0.78) {repeat a color only\\if row conflicts vanish};
  \end{scope}
