...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[paper box, fill=paperdiagblue, text width=3.8cm] (main) at (5.7,0)
    {\textbf{JAX+PETSc}\\\textbf{realization}\\Newton--Krylov + PMG};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[paper box, fill=paperdiaggreen, text width=15.2cm, below=0.45cm of e] (c)
    {\textbf{Constitutive AD}\\$\psi(\varepsilon_q)$; $B_q^\top C_q B_q$ assembly; lowest-cost route in the fixed 3D endpoint comparison};
  \node[paper box, fill=paperdiagrose, text width=15.2cm, below=0.45cm of c] (s)
    {\textbf{Colored SFD}\\rank-local probe HVPs only where needed; parallel coloring / recovery; useful when exact Hessians are too expensive};
\end{tikzpicture}%
}
