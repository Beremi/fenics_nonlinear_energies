...
objective minimisation, we write its discrete functional as
$\mathcal{J}_h(u_h)$; when a family is posed as a sequence of equilibrium
solves, we write the associated discrete total potential as $\Pi_h(u_h)$.
...
potential, a finite-strain stored-energy density, a plasticity potential, or
the reduced design objective used in topology optimisation. The benchmark
sections specialise \cref{eq:generic-objective,eq:generic-potential} to each
...
element Hessian is substantially more expensive than the $6\times6$
...
This hybrid logic follows the implementation documented in the repo note on
trust-region and line-search coupling.
...
is simpler: Newton with Armijo line search and a PMG-preconditioned
\code{fgmres} linear solve. This is the most stable and interpretable converged
story currently available for the flagship benchmark reported later in
...
This keeps the scalar-energy contract intact while reducing the cost of exact
second-order information on high-order tetrahedra.
...
The key methodological point is that this is still exact AD on a scalar
potential. The code does not switch to a hand-derived constitutive tangent. It
changes where automatic differentiation is applied so that high-order 3D
plasticity remains computationally viable.
