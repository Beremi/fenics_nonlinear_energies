...
\end{align}
The showcase endpoint runs in this paper use $\lambda_{\mathrm{sr}} = 1$ and
...
validation ladder. Direct-branch reference-observable agreement for the
endpoint surrogate is treated separately from a fixed-$\lambda_{\mathrm{sr}}$
reference-operator comparison. The latter checks reference-family behavior under matched geometry,
...
reference-family context, not as evidence that this endpoint surrogate is a full
...
\begin{figure}[H]
  ...
  glued-bottom Plasticity3D study runs at
...
\begin{figure}[H]
  ...
  runs use the corrected glued-bottom solver stack at
