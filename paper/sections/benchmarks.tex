...
benchmark. For a reference point $X=(X_1,X_2,X_3)$ on the right face and
$N=24$ reported load steps, the prescribed position at step $k$ is
\[
  \bar y_k(X)
  =
...
The benchmark uses a Davis-B-style reduction path to map the raw
...
$\psi = 0^\circ$, so the reported states should be read as algorithmic endpoint
...
\end{equation}
The studied discrete model uses a heterogeneous source-family slope mesh
hierarchy with same-mesh tetrahedral $P_1/P_2/P_4$ spaces, component-wise
displacement boundary labels, and an additional glued-bottom condition that
...
The local constitutive surrogate follows the source-family piecewise branch
structure. At one quadrature point we suppress the index $q$ and write
...
validation ladder. Direct-branch source-observable agreement for the endpoint
surrogate is treated separately from a fixed-$\lambda_{\mathrm{sr}}$
reference-operator comparison. The latter checks source-family behavior under matched geometry,
...
source-family context, not as evidence that this endpoint surrogate is a full
...
and prescribed traction on a right-edge patch. The reduced objective is
\begin{align}
  \mathcal{J}_h(z_h)
  &=
  \sum_{e=1}^{n_{\mathrm{el}}}
...
previous mechanics state, $\lambda_V$ weights the volume term,
$\alpha_{\mathrm{reg}}$ and $\ell_{\mathrm{pf}}$ define the phase-field-type
regularization scale, and $\mu_{\mathrm{move}}$ weights the latent-field move
penalty. These are algorithmic design choices rather than standard SIMP
notation. The outer loop alternates mechanics solves for
