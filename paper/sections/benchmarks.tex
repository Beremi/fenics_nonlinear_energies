...
the two-dimensional Mohr--Coulomb endpoint surrogate. The dilation angle enters
this endpoint model only through the Davis-B reduction below.
...
SIMP exponent. For fixed density, the discretization solves for the linear elastic
displacement state
$u_h(\theta_h)$ solving
\begin{equation}
  a_{\theta_h}(u_h,v_h)
  ...
  \sum_{e=1}^{n_{\mathrm{el}}}
    |e|\,\theta_e^{p}
    \varepsilon(u_h)^\top C \varepsilon(v_h)
  =
  \ell(v_h)
  \qquad \forall v_h \in V_{h,0},
...
\[
  ...
  \varepsilon(u_h)^\top C\varepsilon(u_h).
\]
