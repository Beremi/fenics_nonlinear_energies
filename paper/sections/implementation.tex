...
solver-stack issues. In the current repository they are used mainly as
...
\begin{itemize}
  ...
  \item \textbf{local SFD / colored SFD}: sparse approximate Hessian recovery
        when exact second-order information is not the best cost tradeoff.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Plasticity (2D):} deep-tail multigrid hierarchies whose
        bottlenecks are now dominated by the top smoother and repeated Krylov
        work.
