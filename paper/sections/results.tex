...
Across the scalar and hyperelasticity rows, local colored SFD reaches the same
reported energy and identical nonlinear/Krylov iteration counts, but with a
clear wall-time premium. The Plasticity3D endpoint favors constitutive AD:
constitutive AD is fastest, element AD also converges, and colored SFD reaches
the same reported energy but exhausts the strict trust-region convergence gate
before satisfying $\|\nabla J\|_2 < \num{1e-4}$. The failed SFD row is retained
because it is part of the comparison outcome, not a missing measurement.
...
\begin{table}[H]
  ...
  subgraph finite-difference Hessian recovery.}
