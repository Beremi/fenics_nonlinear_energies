...
choice on four maintained local benchmarks using the same element assembly path
and fixed preconditioner family within each problem. The scalar cases use
level-10 meshes obtained by one additional uniform refinement of the maintained
level-9 inputs. The $p$-Laplace row uses 32 MPI ranks with a 30 s launcher cap;
...
$\|\nabla J\|_2 < 10^{-4}$ gate, line-search-only Newton reaches the iteration
...
\paragraph{Maintained discretization and scaling.}
