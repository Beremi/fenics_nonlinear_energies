"""
DOF-based domain decomposition for parallel JAX energy computation.
...
Communication needed: Allgatherv of free-DOF vector u before each
computation (to fill ghost values).  This is the same Allgatherv the
existing ParallelSFDSolver already does.
...
class DOFPartition:
    """DOF-based domain decomposition for one MPI rank.

    After construction, the rank has:
      - ``lo, hi``:  owned free-DOF range [lo, hi) in **reordered** space
      - ``elems_local``:  local element connectivity (local node indices)
      - ``dvx_local, dvy_local, vol_local``:  local element data
      - ``elem_weights``:  1.0 for uniquely-owned elements, 0.0 otherwise (energy)
      - ``owned_local_indices``:  local indices of owned free DOFs
      - ``v_template``:  pre-filled Dirichlet values (numpy)
      - methods to build v_local from u_full (u_full is in reordered order)
    ...
    def __init__(self, params, comm, adjacency=None, reorder=True):
        ...
        f = np.asarray(params["f"], dtype=np.float64)
        ...
        # total_to_free_reord[total_node] = reordered free DOF index, or -1
        ...
        # ---- 6. Local element data ----
        local_elems_total = elems[local_elem_idx]
        self.dvx_np = dvx[local_elem_idx].astype(np.float64)
        self.dvy_np = dvy[local_elem_idx].astype(np.float64)
        self.vol_np = vol[local_elem_idx].astype(np.float64)
        ...
        # Owned original free DOF indices: perm[lo:hi]
        # f values at those DOFs: f[freedofs[perm[lo:hi]]]
        owned_orig_free = perm[lo:hi]
        self.f_owned = f[freedofs[owned_orig_free]].astype(np.float64)
    ...
    def _setup_ghost_exchange(self):
        ...
        Instead of Allgatherv of the full free-DOF vector (~6 MB at level 9),
        each rank exchanges ONLY the ghost DOF values it needs from neighbour
        ranks.  At 16 ranks the ghost layer is ~1 % of local DOFs, so the
        data volume drops from 6 MB to ~8-32 KB per message.
        """
    ...
    def _p2p_fill(self, template, u_owned, tag):
        ...
        tag : int
            MPI tag for the P2P exchange (use distinct tags for concurrent
            exchanges, e.g. 42 for v_local and 43 for t_local).
        """
    ...
    def build_v_local_p2p(self, u_owned):
        ...
        Much cheaper than Allgatherv at high rank counts (exchanges ~1 %
        ghost layer instead of full 784K-DOF vector).
        """
    ...
    def build_zero_local_p2p(self, u_owned):
        ...
        ghost free DOFs via point-to-point Isend/Irecv.
        Used for tangent vectors in HVP computation.
        """
