"""Shared simplex Lagrange ordering and basis helpers for 3D same-mesh tets."""
...
TET_EDGE_ORDER: tuple[tuple[int, int], ...] = (
    (0, 1),
    (1, 2),
    (0, 2),
    (1, 3),
...
TET_FACE_ORDER: tuple[tuple[int, int, int], ...] = (
    (0, 1, 2),
    (0, 1, 3),
    (0, 2, 3),
    (1, 2, 3),
...
def triangle_reference_nodes(order: int) -> np.ndarray:
    p = float(order)
    tuples = triangle_lagrange_node_tuples(int(order))
    out = np.empty((2, len(tuples)), dtype=np.float64)
    for idx, counts in enumerate(tuples):
        out[:, idx] = np.array([counts[1], counts[2]], dtype=np.float64) / p
    return out


def _lagrange_factor_and_derivative(
    order: int,
    degree: int,
    lam: np.ndarray,
...
def evaluate_tetra_lagrange_basis(
    order: int,
    xi: np.ndarray,
    ...
    if xi_arr.ndim != 2 or xi_arr.shape[0] != 3:
        raise ValueError(
            f"Expected tetrahedral points with shape (3, n_q), got {xi_arr.shape}."
        )

    xi1 = xi_arr[0, :]
    xi2 = xi_arr[1, :]
    xi3 = xi_arr[2, :]
    bary = (1.0 - xi1 - xi2 - xi3, xi1, xi2, xi3)
    tuples = tetra_lagrange_node_tuples(int(order))
    n_q = xi_arr.shape[1]
    n_p = len(tuples)
...
def evaluate_triangle_lagrange_basis(
    order: int,
    xi: np.ndarray,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    xi_arr = np.asarray(xi, dtype=np.float64)
    if xi_arr.ndim != 2 or xi_arr.shape[0] != 2:
        raise ValueError(
            f"Expected triangular points with shape (2, n_q), got {xi_arr.shape}."
        )

    xi1 = xi_arr[0, :]
    xi2 = xi_arr[1, :]
    bary = (1.0 - xi1 - xi2, xi1, xi2)
    tuples = triangle_lagrange_node_tuples(int(order))
    n_q = xi_arr.shape[1]
    n_p = len(tuples)
    ...
    for idx, counts in enumerate(tuples):
        ...
        hatp[idx, :] = factors[0] * factors[1] * factors[2]
        dhat1[idx, :] = dlam[1] - dlam[0]
        dhat2[idx, :] = dlam[2] - dlam[0]
