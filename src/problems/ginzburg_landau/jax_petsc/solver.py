"""Ginzburg-Landau 2D solver logic for JAX + PETSc backends."""
...
from src.problems.ginzburg_landau.jax_petsc.parallel_hessian_dof import (
...
def _mesh_loader(level: int):
    mesh_obj = MeshGL2D(level)
    return mesh_obj.get_data_jax()


def _linear_record_extras(ksp: PETSc.KSP, rhs: PETSc.Vec) -> dict[str, object]:
    reason_code = int(ksp.getConvergedReason())
    return {
        "ksp_reason_code": int(reason_code),
        "ksp_reason_name": ksp_reason_name(reason_code),
...
SPEC = ScalarProblemDriverSpec(
    ...
    assembler_factories={
        "element": GLReorderedElementAssembler,
    ...
    linear_record_extras=_linear_record_extras,
)
