#!/usr/bin/env python3
"""Ginzburg-Landau 2D solver — DOF-partitioned JAX + PETSc CLI."""
...
def _build_parser(profile_defaults):
    parser = argparse.ArgumentParser(
        description="GinzburgLandau2D DOF-partitioned JAX + PETSc solver"
    )

    parser.add_argument("--level", type=int, default=9, help="Mesh level (5-9)")
    parser.add_argument(
        "--profile",
    ...
    parser.add_argument("--gamg_threshold", type=float, default=None, help="GAMG threshold")
    parser.add_argument(
        "--gamg_agg_nsmooths",
        type=int,
        default=None,
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Set PC coordinates when using GAMG",
    )
    parser.add_argument(
        "--reorder",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Enable/disable RCM reordering in the generic partition path",
    )
    ...
    parser.add_argument(
        ...
        choices=("sfd", "element"),
        default="sfd",
        help="Hessian assembly mode: 'sfd' or 'element'",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help="Element mode only: local Hessian kernel",
    )

    parser.add_argument("--tolf", type=float, default=1e-6, help="Energy-change tolerance")
    parser.add_argument("--tolg", type=float, default=1e-5, help="Gradient-norm tolerance")
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Use rho-based trust region",
    )
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Apply a post-KSP line search for trust-region KSPs",
    )
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Retry once with tighter linear settings",
    )
    parser.add_argument("--nproc", type=int, default=1, help="OMP thread count per rank")
    parser.add_argument("--save_history", action="store_true", help="Save Newton history")
    parser.add_argument(
        "--save_linear_timing",
        action="store_true",
...
def main():
    ...
    from src.problems.ginzburg_landau.jax_petsc.solver import PROFILE_DEFAULTS, run
