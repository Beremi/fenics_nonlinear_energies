"""Nested-grid error helpers for the structured unit-square meshes."""

from __future__ import annotations

import numpy as np

from src.problems.plaplace_up_arctan.functionals import expand_free_vector, seminorm_full
...
def prolong_free_to_problem(
    ...
    if str(source_params["topology"]) != str(target_params["topology"]):
...
def nested_w1p_error(
    ...
    return float(seminorm_full(target_params, diff, exponent=float(target_params["p"])))
