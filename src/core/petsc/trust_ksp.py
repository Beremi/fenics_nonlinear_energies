...
def _get_libpetsc() -> ctypes.CDLL:
    ...
    try:
        from petsc4py import PETSc  # Imported lazily to keep module import cheap.

        # petsc4py already links against the exact PETSc shared object in use.
        search_paths.extend(
            [
                os.path.realpath(PETSc.__file__),
                PETSc.__file__,
            ]
        )
    except Exception:
