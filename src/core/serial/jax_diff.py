...
class EnergyDerivator:
    ...
    def _filter_params(self, params):
        signature = inspect.signature(self.energy_jax_fun)
        if any(
        ...
        accepted = list(signature.parameters.keys())[1:]
        accepted_set = set(accepted)
        return {key: value for key, value in params.items() if key in accepted_set}
