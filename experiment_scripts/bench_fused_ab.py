#!/usr/bin/env python3
"""A/B benchmark: fused grad+HVP (jax.linearize) vs separate computation.

Usage:
  docker exec bench_container bash -c "OMP_NUM_THREADS=1 mpirun -np 16 python -m experiment_scripts.bench_fused_ab --level 9"
"""
...
def run_one(label, fuse, trial_idx):
    gc.collect()
    comm.Barrier()
    ...
    # Toggle fused computation
    assembler.fuse_grad_hvp = fuse
    ...
    n_fused = sum(1 for d in assembler.iter_timings if d.get("fused", False))

    assembler.cleanup()

    return {
        "label": label, "fuse": fuse,
...
all_results = {}
for label, fuse in configs:
    runs = []
    for i in range(args.repeats):
        r = run_one(label, fuse, i)
        runs.append(r)
        if rank == 0:
            sys.stdout.write(
                f"  [{label:>8s}] rep {i + 1}/{args.repeats}  "
                f"setup={r['setup']:.3f}  solve={r['solve']:.3f}  "
                f"total={r['total']:.3f}  newton={r['newton_its']}  "
                f"ksp={r['ksp_its']}  ls={r['ls_evals']}  "
                f"fused_its={r['n_fused']}  J={r['energy']:.6f}\n"
            )
            sys.stdout.flush()
...
if rank == 0:
    sys.stdout.write("\n" + "=" * 110 + "\n")
    hdr = (
        f"{'Config':<12s} {'setup':>7s} {'solve':>7s} {'total':>7s} "
        f"{'N.its':>5s} {'KSP':>5s} {'LS':>5s} {'t_ksp':>7s} "
        f"{'t_hvp':>7s} {'t_grad':>7s} {'t_ls':>7s} {'t_hess':>7s} "
        f"{'g+h':>7s} {'fused':>5s} {'J(u)':>12s}"
    )
    ...
    for label, runs in all_results.items():
        ...
        g_plus_h = r['t_grad'] + r['t_hvp']
        sys.stdout.write(
            f"{r['label']:<12s} {r['setup']:7.3f} {r['solve']:7.3f} {r['total']:7.3f} "
            f"{r['newton_its']:5d} {r['ksp_its']:5d} {r['ls_evals']:5d} {r['t_ksp']:7.3f} "
            f"{r['t_hvp']:7.3f} {r['t_grad']:7.3f} {r['t_ls']:7.3f} {r['t_hess']:7.3f} "
            f"{g_plus_h:7.3f} {r['n_fused']:5d} {r['energy']:12.6f}\n"
        )
    sys.stdout.write("=" * 110 + "\n")
