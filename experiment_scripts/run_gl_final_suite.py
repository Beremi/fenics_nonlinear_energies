#!/usr/bin/env python3
"""Run the final GL benchmark suite with frozen settings."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-dir",
        type=str,
        default="experiment_results_cache/gl_final_suite",
    )
    parser.add_argument(
        "--resume",
    ...
    parser.add_argument("--tolg", type=float, default=1e-5)
...
def _summarize_case(solver_name: str, level: int, nprocs: int, payload: dict) -> dict:
...
def main() -> None:
    ...
    solver_cfgs = [_solver_config(name) for name in args.solvers]

    for level in args.levels:
        for nprocs in args.nprocs:
            for solver in solver_cfgs:
                if (solver["name"], int(level), int(nprocs)) in done:
                    continue
                row = _run_case(repo_root, out_dir, solver, level, nprocs, args)
                rows.append(row)
                rows.sort(key=lambda r: (r["solver"], int(r["level"]), int(r["nprocs"])))
