#!/usr/bin/env python3
"""Sweep GL minimizer settings on the finest mesh benchmark."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-dir",
        type=str,
        default="experiment_results_cache/gl_minimizer_sweep_l9_np32",
    )
    ...
    for solver in SOLVERS:
        for config in CASES:
            ...
            if timed_out or not case_json.exists():
                ...
                rows.append(_summarize(payload, solver["name"], config["name"]))

            rows.sort(key=lambda row: (row["solver"], row["config"]))
