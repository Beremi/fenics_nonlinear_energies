#!/usr/bin/env python3
"""Full parallel test of best SNES config for GL."""
import sys
import numpy as np
...
if rank == 0:
    sys.stdout.write("GL SNES (basic+gmres+hypre, divtol=-1) | {} procs\n".format(nprocs))
    sys.stdout.flush()
    ...
    if rank == 0:
        ...
        sys.stdout.write(
            "  level={} dofs={}: time={:.3f}s iters={} reason={} J={:.6f} {}\n".format(
                mesh_level, V.dofmap.index_map.size_global, dt, n_iters, reason, final_energy,
                "OK" if ok else "FAIL"
            )
        )
        sys.stdout.flush()
