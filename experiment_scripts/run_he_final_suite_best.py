...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-dir",
        type=str,
        default="experiment_results_cache/he_final_suite_stcg_best",
    )
    parser.add_argument(
        "--resume",
...
def _write_case_markdown(case_name: str, md_path: Path, payload: dict, row: dict, repo_root: Path) -> None:
    ...
    lines = [
        ...
        f"| Raw JSON | `{md_path.with_suffix('.json').relative_to(repo_root)}` |",
        f"| Raw log | `{md_path.with_suffix('.log').relative_to(repo_root)}` |",
        "",
        "## Frozen Settings",
        "",
        "| Setting | Value |",
...
def _write_summary_markdown(path: Path, rows: list[dict], repo_root: Path) -> None:
    ...
    for row in rows:
        ...
        json_rel = Path(json_path).relative_to(repo_root) if json_path else None
        md_rel = Path(md_path).relative_to(repo_root) if md_path else None
        lines.append(
...
def _seed_known_results(
    ...
    _seed_exact_result(
        ...
        level=4,
        nprocs=32,
        src_json_rel="experiment_results_cache/he_stcg_ksp_compare_l4_np32/fenics_ksp30.json",
        src_log_rel="experiment_results_cache/he_stcg_ksp_compare_l4_np32/fenics_ksp30.log",
        args=args,
    )
    _seed_exact_result(
        repo_root,
        ...
        level=4,
        nprocs=32,
        src_json_rel="experiment_results_cache/he_stcg_ksp_compare_l4_np32/jax_ksp30.json",
        src_log_rel="experiment_results_cache/he_stcg_ksp_compare_l4_np32/jax_ksp30.log",
        args=args,
    )
