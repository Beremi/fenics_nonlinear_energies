#!/usr/bin/env python3
"""
Benchmark C3: newtonls + basic + EW + lag_pc=2 + fgmres + HYPRE with different ksp_rtol values.
Levels 5-9, 3 reps, early stop on failure.
...
rank = comm.rank
nprocs = comm.size
KSP_RTOL = float(os.environ.get("KSP_RTOL", "1e-6"))
...
def solve_gl(mesh_level, max_reps=3):
    ...
    opts = {
        ...
        "snes_atol": 1e-5, "snes_rtol": 1e-8, "snes_max_it": 100,
        "snes_divergence_tolerance": -1.0,
        "ksp_type": "fgmres", "ksp_rtol": KSP_RTOL, "ksp_max_it": 200,
        "pc_type": "hypre", "pc_hypre_type": "boomeramg",
    }
    pfx = "c3{}_{}{}_".format(str(KSP_RTOL).replace('.', ''), nprocs, mesh_level)
    problem = NonlinearProblem(J_form, u, petsc_options_prefix=pfx, bcs=[bc],
                               petsc_options=opts)
    # No SNESSetObjective for C3 (original config didn't use it)
...
for lvl in [5, 6, 7, 8, 9]:
    ndofs, (dt, iters, reason, energy) = solve_gl(lvl, max_reps=3)
    if rank == 0:
        ...
        outfile = "/work/experiment_scripts/c3_ksp{}_np{}.txt".format(
